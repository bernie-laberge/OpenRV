...
//  ------------------------------------------
//
//  If you implement an output plugin, you will need to have a function called
//  "output_module_create" which is extern "C" and has the signature:
//
//      extern "C" TwkApp::VideoModule* output_module_create(float output_plugin_version, int output_module_index);
//
...
class OutputPlugin
{
  public:
    ...
    OutputPlugin(const std::string& file, 
                 void* handle, 
    ...
    static TwkApp::OutputPlugin* loadPlugin(const std::string& pathToPlugin);
